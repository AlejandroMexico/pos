...
/**
     *    check if exists this code in another terminal
     * @param      strin code       code to check
     * @param        string code        Code of cash
     * @return     int     terminal id or null
     */
    function checkcode($code, $ref = '')
    {
        global $conf;
       ...
       $sql .= " AND code =\"" . $code. "\"";
        ...
        dol_syslog("Cash::checkcode sql=" . $sql, LOG_DEBUG);
        $result = $this->db->query($sql);
        if ($result) {
            if ($this->db->num_rows($result)) {
                $obj = $this->db->fetch_object($result);
     ...
     * Lock terminals
     *
     * @param        User $User User who blocks terminals
     *
     * @return        int $Wharehouse  optional, block only for specific warehouse
     */

    function lock_all($user, $warehouse = null)
    {
            dol_syslog(get_class($this) . "::Lock_terminals=" . $user->id, LOG_DEBUG);
            $this->db->begin();
            $sql = 'UPDATE ' . MAIN_DB_PREFIX . 'pos_cash SET';
            $sql .= ' is_used=1';
            $sql .= ', is_closed=0';
            $sql .= ", fk_user_u=" . $user->id;
            $sql .=  (isset($warehouse) ? " WHERE fk_warehouse=".$warehouse : "") . ";";
            $resql = $this->db->query($sql);
            if (!$resql) {
                $error++;
                $this->error = $this->db->error();
    ...
    function unlock_all($warehouse = null)
    {
            dol_syslog(get_class($this) . "::Unlock_terminals=" . $user->id, LOG_DEBUG);
            $this->db->begin();
            $sql = 'UPDATE ' . MAIN_DB_PREFIX . 'pos_cash SET';
            $sql .= ' is_used=0';
            $sql .= ', is_closed=0';
...
class ControlCash extends CommonObject
{
    ...
    function Create($data)
    {
        ...
        if ($resql) {
            ...
            if ($data['type_control'] == 1) {
                ...
                $res = $ticket->delete($this->terminal);
    ...
    function getMoneyCash($open = false)
    {
        ...
        return $this->getMoneyFromAccount($cash->fk_paycash);

    }

    function getMoneyFromAccount($account)
    {
        global $db, $conf;

        $sql = "SELECT sum( b.amount ) as amount";
        $sql .= " FROM " . MAIN_DB_PREFIX . "bank_account AS ba, " . MAIN_DB_PREFIX . "bank AS b";
        $sql .= " WHERE b.fk_account =" . $account;
