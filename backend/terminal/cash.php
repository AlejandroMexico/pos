...
$sql = "SELECT p.rowid, p.code, p.name, p.tactil, p.is_used, p.is_closed, p.fk_user_u, u.firstname, u.lastname ";

$sql.= " FROM ".MAIN_DB_PREFIX."pos_cash as p";
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."user as u";
$sql.= " ON fk_user_u = u.rowid";

$sql.= " WHERE p.entity = ".$conf->entity;

if ($cashid)	$sql.= " AND p.rowid = ".$cashid;

if ($search_name)
{
	$sql.= " AND (";
	$sql.= "p.name LIKE '%".$db->escape($search_name)."%'";
	$sql.= ")";
}
...
if ($resql)
{
	...
	print_liste_field_titre($langs->trans("Code"),$_SERVER["PHP_SELF"],"code","",$params,"",$sortfield,$sortorder);	
...
//	print_liste_field_titre($langs->trans("Status"),$_SERVER["PHP_SELF"],"s.status","",$params,'align="right"',$sortfield,$sortorder);
	...
	while ($i < min($num,$conf->liste_limit))
	{
		...
		$cashwil = new Cash($db);
		$cashwil->id = $obj->rowid;
		$cashwil->name = $obj->name;
		$cashwil->is_used = $obj->is_used;
		$cashwil->is_closed = $obj->is_closed;
