		...
		if ($result)
		{
			...
			if (! empty($object->lines))
			{
				...
				foreach ($object->lines as $line)
				{
					...
					echo ('<tr><td align="left">'.$line->libelle.'</td><td align="left">'.$line->qty." * ".price(($conf->global->POS_TICKET_TTC?$line->subprice*(1+$line->tva_tx/100):$line->subprice)).'</td>'.($onediscount?'<td align="right">'.$line->remise_percent.'%</td>':'').'<td class="total">'.price(($conf->global->POS_TICKET_TTC?$line->total_ttc:$line->total_ht)).'</td></tr>');
