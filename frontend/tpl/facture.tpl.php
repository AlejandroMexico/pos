...
<div class="entete">
	...
	<div class="infos">
		...
		<?php
			...
			echo $client->name.'<br>';
			echo (!empty($client->idprof1)?$client->idprof1.'<br>':'');
			echo (!empty($client->address)?$client->address.'<br>':'');
			echo $client->zip.' '.$client->town.'<br>';
			echo $client->state.'</p>';
		...
		if ($result)
		{
			...
			if (! empty($object->lines))
			{
				...
				foreach ($object->lines as $line)
				{
					...
					if($are_promo){
						echo ('<tr><td align="left">'.$line->product_label.'</td><td align="left">'.$line->qty." * ".price(price2num($conf->global->POS_TICKET_TTC?$dis_doc->ori_subprice*(1+$line->tva_tx/100):$dis_doc->ori_subprice)).'</td>'.($onediscount?'<td align="right">'.$line->remise_percent.'%</td>':'').'<td class="total">'.price(price2num($conf->global->POS_TICKET_TTC?$dis_doc->ori_totalht*(1+$line->tva_tx/100):$dis_doc->ori_totalht)).'</td></tr>');
						echo ('<tr><td align="left">'.$langs->trans("InPromo").'</td><td align="left"></td>'.($onediscount?'<td align="right"></td>':'').'<td class="total">-'.price(price2num($conf->global->POS_TICKET_TTC?$dis_doc->ori_totalht*(1+$line->tva_tx/100) - $line->total_ttc:$dis_doc->ori_totalht - $line->total_ht)).'</td></tr>');
						...
						echo ('<tr><td align="left">'.$line->product_label.'</td><td align="left">'.$line->qty." * ".price(price2num($conf->global->POS_TICKET_TTC?$line->subprice*(1+$line->tva_tx/100):$line->subprice)).'</td>'.($onediscount?'<td align="right">'.$line->remise_percent.'%</td>':'').'<td class="total">'.price(price2num($conf->global->POS_TICKET_TTC?$line->total_ttc:$line->total_ht)).'</td></tr>');
					}
