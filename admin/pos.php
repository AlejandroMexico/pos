...
if (GETPOST("action") == 'set') {
	...
	if (GETPOST("POS_FACTURE", "int") == 0 && $conf->global->POS_TICKET == 0) {
		...
		$res = dolibarr_set_const($db, "POS_SELL_CREDIT", GETPOST("POS_SELL_CREDIT"), 'chaine', 0, '', $conf->entity);

		if (!$res > 0) {
			$error++;
		}

		$res = dolibarr_set_const($db, "POS_SEARCH_PRODUCT_LIMIT", GETPOST("POS_SEARCH_PRODUCT_LIMIT"), 'chaine', 0, '', $conf->entity);
...
$var = !$var;
print '<tr ' . $bc[$var] . '><td>';
print $langs->trans("POSSellCredit");
print '<td colspan="2">';
print $html->selectyesno("POS_SELL_CREDIT", $conf->global->POS_SELL_CREDIT, 1);
print "</td></tr>\n";

$var = !$var;
print '<tr ' . $bc[$var] . '><td>';
print $langs->trans("POSSearchProductLimit");
print '<td colspan="2" align="left">';
print '<input size="3" type="text" class="flat" id="POS_SEARCH_PRODUCT_LIMIT" name="POS_SEARCH_PRODUCT_LIMIT" value="'.$conf->global->POS_SEARCH_PRODUCT_LIMIT.'">';
print "</td></tr>\n";
